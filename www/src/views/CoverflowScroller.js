define(function(require, exports, module) {
    ...
    /**
     * CoverflowScroller lays out a collection of renderables, and will browse through them based on
     * accessed position. CoverflowScroller also broadcasts an 'edgeHit' event, with a position property of the location of the edge,
     * when you've hit the 'edges' of it's renderable collection.
     * @class CoverflowScroller
     * @constructor
      * @event error
     * @param {Options} [options] An object of configurable options.
     * @param {Number} [options.direction=Utility.Direction.Y] Using the direction helper found in the famous Utility
     * module, this option will lay out the CoverflowScroller instance's renderables either horizontally
     * (x) or vertically (y). Utility's direction is essentially either zero (X) or one (Y), so feel free
     * to just use integers as well.
     * @param {Number} [clipSize=undefined] The size of the area (in pixels) that CoverflowScroller will display content in.
     * @param {Number} [margin=undefined] The size of the area (in pixels) that CoverflowScroller will process renderables' associated calculations in.
     */
    function CoverflowScroller(options) {
    ...
    CoverflowScroller.DEFAULT_OPTIONS = {
    ...
    /**
     * Patches the CoverflowScroller instance's options with the passed-in ones.
     * @method setOptions
     * @param {Options} options An object of configurable options for the CoverflowScroller instance.
     */
    CoverflowScroller.prototype.setOptions = function setOptions(options) {
    ...
    /**
     * Tells you if the CoverflowScroller instance is on an edge.
     * @method onEdge
     * @return {Boolean} Whether the CoverflowScroller instance is on an edge or not.
     */
    CoverflowScroller.prototype.onEdge = function onEdge() {
        return this._onEdge;
    };

    /**
     * Allows you to overwrite the way CoverflowScroller lays out it's renderables. CoverflowScroller will
     * pass an offset into the function. By default the CoverflowScroller instance just translates each node
     * in it's direction by the passed-in offset.
     * CoverflowScroller will translate each renderable down
     * @method outputFrom
     * @param {Function} fn A function that takes an offset and returns a transform.
     * @param {Function} [masterFn]
     */
    CoverflowScroller.prototype.outputFrom = function outputFrom(fn, masterFn) {
        if (!fn) {
            fn = function(offset) {
    ...
    /**
     * The CoverflowScroller instance's method for reading from an external position. CoverflowScroller uses
     ...
     */
    CoverflowScroller.prototype.positionFrom = function positionFrom(position) {
    ...
    /**
     * Sets the collection of renderables under the CoverflowScroller instance's control.
     *
     * @method sequenceFrom
     * @param node {Array|ViewSequence} Either an array of renderables or a Famous viewSequence.
     * @chainable
     */
    CoverflowScroller.prototype.sequenceFrom = function sequenceFrom(node) {
        if (node instanceof Array) node = new ViewSequence({array: node});
        this._node = node;
        this._positionOffset = -this.options.screenCenter;
    };

    /**
     * Returns the width and the height of the CoverflowScroller instance.
     *
     * @method getSize
     * @return {Array} A two value array of the CoverflowScroller instance's current width and height (in that order).
     */
    CoverflowScroller.prototype.getSize = function getSize(actual) {
     ...
     */
    CoverflowScroller.prototype.render = function render() {
     ...
     */
    CoverflowScroller.prototype.commit = function commit(context) {
    ...
    function _innerRender() {
        ...
        var result = [];
        var center = this.options.screenCenter;
        ...
        if (!currNode && offset - position < clipSize - EDGE_TOLERANCE - center) {
            if (this._onEdge !== 1){
                this._onEdge = 1;
                this._eventOutput.emit('onEdge', {
                    position: offset - clipSize + center
                });
            }
        }
...
//        _normalizeState.call(this);
    ...
    module.exports = CoverflowScroller;
});
